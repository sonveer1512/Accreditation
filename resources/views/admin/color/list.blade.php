        ...
        <div class="row g-2">
            ...
            <div class="col-sm-auto ms-auto">
                <div class="list-grid-nav gap-1">
                    <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addmembers"><i class="ri-add-fill me-1 align-bottom"></i> Add Colors</button>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card-body">
                <div id="customerList">
                    ...
                    <div class="table-responsive table-card mt-3 mb-1">
                        @php if (!empty($color)) { @endphp
                        <table class="table align-middle table-nowrap" id="customerTable">
                            <thead class="table-light">
                                <tr>
                                    <th class="sort">S. No.</th>
                                    <th class="sort" data-sort="customer_name">Title</th>
                                    <th class="sort" data-sort="email">Color Code</th>
                                    <th class="sort" data-sort="action">Action</th>
                                </tr>
                            </thead>
                            <tbody class="list form-check-all">
                                @php $i = 0; foreach ($color as $value) { $i++; @endphp
                                <tr>
                                    <td><?= $i ?></td>
                                    <td class="email">{{ $value->title ?? '' }}</td>
                                    <td class="email">{{ $value->color_code ?? '' }}</td>
                                    <td>
                                        ...
                                        @if ($value->flag == 1)
                                        <button class="btn btn-sm btn-success" data-bs-target="#deletepopup" data-bs-toggle="modal" onclick="deletepopup(<?= $value->id ?>,'colors','activate')"><i class="ri-checkbox-circle-line"></i></button>
                                        @else
                                        <button class="btn btn-sm btn-danger" data-bs-target="#deletepopup" data-bs-toggle="modal" onclick="deletepopup(<?= $value->id ?>,'colors','deactivate')"><i class="ri-alert-line"></i></button>
                                        @endif

                                        <button class="btn btn-sm btn-danger remove-item-btn" data-bs-target="#deletepopup" data-bs-toggle="modal" onclick="deletepopup(<?= $value->id ?>,'colors','delete')"><i class="ri-delete-bin-line"></i></button>
                                    </td>
                                </tr>
                                @php } @endphp
                            </tbody>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-soft-info p-3">
                <h5 class="modal-title" id="myModalLabel">Add New Color</h5>
...
<div class="modal fade" id="editmodal" tabindex="-1" aria-labelledby="exampleModalgridLabel" aria-modal="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-soft-info p-3">
                <h5 class="modal-title" id="myModalLabel">Edit color</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form method="POST" id="editdetails">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="hstack gap-2 justify-content-end">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Close</button>
                                <button type="submit" class="btn btn-success">Edit color</button>
                            </div>
                        </div>
                    </div>
